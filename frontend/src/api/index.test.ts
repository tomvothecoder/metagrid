...
import {
  ...
  rawUserCartFixture,
  rawUserSavedSubscriptionsFixture,
  userAuthFixture,
  userInfoFixture,
...
describe('test fetching user saved subscriptions', () => {
  it('returns user\'s saved subscriptions', async () => {
    const files = await fetchUserSavedSubscriptions('pk', 'access_token');
    expect(files).toEqual(rawUserSavedSubscriptionsFixture());
  });
  it('catches and throws an error based on HTTP status code', async () => {
    server.use(
      rest.get(apiRoutes.userCart.path, (_req, res, ctx) => {
